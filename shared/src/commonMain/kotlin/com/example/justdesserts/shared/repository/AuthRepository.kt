...
class AuthRepository(apolloProvider: AppolloProvider) : BaseRepository(apolloProvider) {
    ...
    suspend fun getProfileDesserts(): List<Dessert> {
        val response = apolloClient.query(GetProfileQuery())
            .execute().single()
        return response.data?.getProfile?.desserts?.map { it.toDessert() }
            ?: emptyList()
    }
